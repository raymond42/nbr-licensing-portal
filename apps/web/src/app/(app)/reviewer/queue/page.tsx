...
export default function ReviewerQueuePage() {
  ...
  const columns: Column<ApplicationDto>[] = [
    ...
    },
    {
      ...
      cell: (r) => <TableViewAction href={`/reviewer/applications/${r.id}`} />,
    },
  ];
  ...
  return (
    <div>
      <PageHeader
        title="Review queue"
        subtitle="Applications awaiting action from your role."
      />
      {q.isPending ? (
        ...
        <DataTable
          ...
          rows={rows}
          getRowKey={(r) => r.id}
          empty={<EmptyState title="No applications in queue" />}
        />
      )}
      {!q.isPending ? (
