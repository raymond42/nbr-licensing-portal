...
- **[`LocatorDevTools`](components/locator-dev-tools.tsx)** loads **`@locator/runtime`** only when `NODE_ENV === 'development'`. Prefer **either** the **browser extension** **or** embedded runtime on a given origin to avoid double injection quirks.
...
- **`@locator/*` versions:** npm registry latest is **0.5.1** (already pinned). Run `pnpm update @locator/runtime @locator/webpack-loader` periodically.
